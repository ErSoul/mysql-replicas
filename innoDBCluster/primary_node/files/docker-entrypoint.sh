...
docker_temp_server_start() {
	if [ "${MYSQL_MAJOR}" = '5.7' ]; then
		"$@" --skip-networking --default-time-zone=SYSTEM &
		...
		if ! "$@" --daemonize --default-time-zone=SYSTEM; then
			mysql_error "Unable to start server."
		fi
	fi
}
...
docker_temp_server_stop() {
	if ! mysqladmin --defaults-extra-file=<( _mysql_passfile ) shutdown -uroot; then
...
docker_process_sql() {
	...
	mysql --defaults-extra-file=<( _mysql_passfile "${passfileArgs[@]}") -uroot -hlocalhost --comments "$@"
}
